...
for res3 in res["statuses"]:
    res4 = res3["user"]
    print(res4["name"], res4["created_at"])

# Notice that each time we descend a level in a dictionary, we have a [] picking out a key. 
# Each time we look inside a list, we will have a for loop. If there are lists at multiple levels, 
# we will have nested for loops.
# Once you’ve figured out how to extract everything you want, you may choose to collapse things with 
# multiple extractions in a single expression. 


# Nested Dictionary extraction
nested_d = {'Beijing':{'China':51, 'USA':36, 'Russia':22, 'Great Britain':19}, 'London':{'USA':46, 'China':38, 'Great Britain':29, 'Russia':22}, 'Rio':{'USA':35, 'Great Britain':22, 'China':20, 'Germany':13}}
US_count=[]
for dict1 in nested_d:
        US_count.append(nested_d[dict1]['USA'])
print(US_count)

# Nested List extraction
athletes = [['Phelps', 'Lochte', 'Schooling', 'Ledecky', 'Franklin'], ['Felix', 'Bolt', 'Gardner', 'Eaton'], ['Biles', 'Douglas', 'Hamm', 'Raisman', 'Mikulak', 'Dalton']]
for athlete1 in athletes:
    for athlete2 in athlete1:
        print (athlete2)        